...
class RegressionTransformerTrainingPipeline():
    ...
    def setup_model(self, params: Dict[str, Any]):
        ...
        if params["model_path"] is None and params["model_type"] is None and params["config_name"] is None:
            raise ValueError(
                "Either pass pretrained model via `model_path` or specify"
                "which model to use via `model_type`."
            )
        ...
        if params["model_path"]:
            ...
            if params["tokenizer_name"].endswith(".txt"):
                self.tokenizer = ExpressionBertTokenizer(vocab_file=params["tokenizer_name"])
            else:
                self.tokenizer = ExpressionBertTokenizer.from_pretrained(
...
if __name__ == "__main__":
    ...
    rt_pipeline = RegressionTransformerTrainingPipeline()
    rt_pipeline.train(
        training_args=training_args.__dict__,
        model_args=model_args.__dict__,
        dataset_args=data_args.__dict__,
    )
