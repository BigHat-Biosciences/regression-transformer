...
from terminator.utils import filter_stubbed, get_substructure_indices, device_claim, map_tensor_dict
...
class ConditionalGenerator:
    ...
    def __init__(
        ...
        self.load_inference(resources_path, inference_config)
    ...
    def load_inference(self, resources_path: str, inference_config=None) -> None:
        """
    ...
    @torch.no_grad()
    def generate_batch_regression(self, contexts: List[Sequence], batch_size=None) -> List[Sequence]:
        """
        Predict the property of a batch of sequences.

        Args:
            contexts: a list of sequences to predict the properties for.
            batch_size: the batch size to use for the prediction. Defaults to self.batch_size.
        ...
        for i in tqdm(range(num_batches)):
            ...
            inputs = self.collator([self.tokenizer(c) for c in batch])
    ...
    def generate_batch_generation(self, sequence: Sequence) -> Tuple:
        ...
        if self.sampling_wrapper != {}:
            sequences = [self.sample_sequence(sequence) for _ in range(self.batch_size)]
            mask_idx = [s[1] for s in sequences]
            sequences = [s[0] for s in sequences]
        
        # The property score has to be in the range understood by the model
        sequences = [self.normalize_sequence(s) for s in sequences]

        # Prepare the batch
        tokens = [self.tokenizer(s) for s in sequences]
        inputs = self.collator(tokens)
        input_ids = inputs["input_ids"].clone()

        if self.search_method in ["greedy", "beam", "sample"]:
            # Forward pass
            outputs = self.model(map_tensor_dict(inputs, self.device))
        ...
        elif self.search_method == "denoise":
            max_masks_per_sequence = max(list(map(len, mask_idx)))

            for mutation_idx in tqdm(range(max_masks_per_sequence)):
                outputs = self.model(map_tensor_dict(inputs, self.device))
                predictions = self.search(outputs["logits"].detach()).cpu()
                
                for i, input_id_row in enumerate(input_ids):
                    shuffled_mask_idx = mask_idx[i]
                    if mutation_idx >= len(shuffled_mask_idx): continue
                        
                    current_masked_idx = shuffled_mask_idx[mutation_idx]
                    assert input_id_row[current_masked_idx] == self.tokenizer.mask_token_id, "Masked token not found at the current masked index"
                    input_id_row[current_masked_idx] = predictions[i][current_masked_idx]
                    inputs["input_ids"] = input_ids
            
            generations = input_ids.cpu()
            assert torch.sum(generations == self.tokenizer.mask_token_id) == 0, "Masked tokens still present in generations tensor"
    ...
    def sample_sequence(self, seq: str) -> str:
        ...
        for i, p in enumerate(self.properties):
            ...
            prop_tokens_length += self.property_mask_lengths[i] + 3                 # <CLS>, <PROP>, <SEP>

        # Return new sequence
        sequence = prop_tokens + "".join(sequence_tokens)
        mask_idxs = [v + prop_tokens_length for v in mask_idxs]
        return sequence, mask_idxs
...
class ProteinLanguageRT(ConditionalGenerator):
    ...
    def __init__(
        ...
        """
        super().__init__(
            device=device, resources_path=resources_path, tolerance=tolerance, inference_config=inference_config
        )
